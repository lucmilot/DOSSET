...
def parse_gen(tree_list,lvl):
    ...
    if keep[lvl][0].is_group: 
        ...
        if isinstance(keepx, IdentifierList):
            yield '\nget identifier from identifierlist \n'
            for t in keepx.get_identifiers():
                yield str(t) + "\n"
        else: 
            yield str(len(keepx.tokens)) + str(keepx)
            yield '\nget list elelement from keepx.tokens \n'
        ...
        yield 'before from parse_gen(keep[' + str(keep[lvl+1]) + "]," + str(lvl+1) + ' ) ' + str(len(keep[lvl+1])) + "\n"
        yield from parse_gen(keep[lvl+1],lvl+1 ) 
        yield 'after from parse_gen(keep[' + str(keep[lvl+1]) + "]," + str(lvl+1) + ' ) ' + str(len(keep[lvl+1])) + "\n"
        ...
        if   re.search(pat_FROM ,str(keepx)) is not None : 
            print(type(keepx).__name__, keepx)
            from_sw = 1
        else :  
            print(type(keepx).__name__, keepx)       
            #if (re.search(pat_view ,str(keepx)) is not None) and (from_sw == 1):     
            if (re.search(pat_view ,str(keepx)) is not None) :      
                from_sw = 0
                print ('view : ' + str(keepx))
                yield 'view : ' + str(keepx)
        yield from parse_gen(keep[lvl],lvl ) 
        

#ttt = 'OY'
#from_sw = 1
#if (re.search(pat_xx ,ttt) is not None) and (from_sw == 1):
#    print('Y')
...
SELECT  'I'                                                       
      ...
      , CARI.CAR_ORD_NBR                                          
      , CARI.OBND_420_BLK                                         
  FROM  OY.TCAR_INVENTORY CARI  ,     OY.TxxxxxORY                               
 WHERE (                                                          
