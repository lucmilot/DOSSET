...
def select_level_2_3(file_list):  
    ...
    def clicked_List_Submit():
        global choice_return
  
        choice_return = file_list[mylist.curselection()[0]]
    ...
    scrollbar = tk.Scrollbar(master2)
    scrollbar.pack( side = tk.RIGHT, fill = tk.Y )
    
    #mylist = tk.Listbox(master2, yscrollcommand = scrollbar.set , width = 30, height = 40)
    listbox1 = tk.Listbox(master2, yscrollcommand = scrollbar.set )
    #mylist.grid(column=1, row=1)
    ...
    #btn = tk.Button(master2, text="Submit", bg="white", fg="green",  height = 2, width = 10, command=clicked_List_Submit)
    #btn.grid(column=2, row=1)
