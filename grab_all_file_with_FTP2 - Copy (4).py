...
for index, row in df_out.iterrows():
    print (row["file"], row["mbr"],row["sql_list"])
    
    
for index, row in df_out.iterrows():
    for j in row['sql_list']
        df_out_1 = df_out.append({'key_%s'%random.choice('abcde'): j}, ignore_index=True)
...
for index, row in df_out.iterrows():
    df_out['sql1'] = df_out.apply(lambda row: row['sql_list'][0] if len(row['sql_list']) > 0 else '')

 

    df_out['sql1']  = df_out.apply(lambda row: row['sql_list'][0] if len(row['sql_list']) > 0 else '')   
    
    df_out['sql1']  = df_out.apply(lambda row: row['file'] ) 
    
    
    xxx = [ x[0] if len(x) > 0 else '' for x in df_out['sql_list'] ]
    
     df_out['sql1']  = xxx
    
        df_out['sql1']  = df_out.apply([ x[0] if len(x) > 0 else '' for x in df_out['sql_list'] ]) 
